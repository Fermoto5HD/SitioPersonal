angular.module('FM5HD')
	...
	.controller('blog_post', ['$http', '$scope', '$routeParams', function ($http, $scope, $routeParams) {
		...
		$http.get('http://Fermoto5HD.com/_JosephIsBestJojo/APIs/blog_post/'+$routeParams.id).success(function(data, status, headers){
			$scope.post = data; 
		});
		...
		$scope.vote = function(vote) {
			...
			$http({
				method: "post", 
				url: 'http://Fermoto5HD.com/_JosephIsBestJojo/privateAPIs/blog_post_vote', 
				data: serialize(data), 
	...
	.factory('fact_blog_posts', ['$http', function($http) {
		return {
			blog_posts: function(callback) {
				$http.get('http://Fermoto5HD.com/_JosephIsBestJojo/APIs/blog').success(function(data){
					callback(data);
				}).error(function(){
					console.error("Error"); 
				});
